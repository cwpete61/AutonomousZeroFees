...
const AGENT_REGISTRY = {
    scout: {
        id: 'scout',
        name: 'Scout Agent',
        description: 'Finds service businesses with poor websites',
    ...
    design_preview: {
        id: 'design_preview',
        name: 'Design Preview Agent',
        description: 'Screenshots existing site, generates redesign mockup with blur',
        triggers: ['lead:outreach_queued'],
        inputs: ['website_url', 'business_name', 'industry'],
        outputs: ['before_screenshot', 'redesign_preview_blurred', 'redesign_preview_full'],
    },
    sales_close: {
        id: 'sales_close',
        name: 'Sales Close Agent',
        description: 'Sends demo on reply, handles objections, sends invoice',
        triggers: ['lead:replied'],
        inputs: ['lead', 'preview_assets', 'pricing_tier'],
        outputs: ['invoice_sent', 'deal_won', 'deal_lost'],
    },
    web_build: {
        id: 'web_build',
        name: 'Web Build Agent',
        description: 'Executes full website build in Antigravity IDE',
        triggers: ['lead:paid'],
        inputs: ['brief', 'brand_assets', 'pages_required'],
        outputs: ['staging_url', 'build_files'],
    },
    client_success: {
        id: 'client_success',
    ...
    async routeLead(lead) {
        ...
        const routingMap = {
            ...
            [PIPELINE_STAGES.OUTREACH_QUEUED]: () => Promise.all([
                this.agents.design_preview.generate(lead),
                this.agents.outreach.sendInitialEmail(lead),
            ]),
            [PIPELINE_STAGES.REPLIED]: () => this.agents.sales_close.sendDemo(lead),
            [PIPELINE_STAGES.INVOICED]: () => this.waitForPayment(lead),
            [PIPELINE_STAGES.PAID]: () => this.agents.web_build.startBuild(lead),
